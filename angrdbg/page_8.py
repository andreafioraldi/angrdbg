...
from angr.errors import SimMemoryError, SimSegfaultError, SimMemoryMissingError
...
class SimDbgMemory(object):
    ...
    def __changed_bytes(self, other):
        ...
        :type other:    SimDbgMemory
        :returns:       A set of differing bytes.
        """
        if self._page_size != other._page_size:
            raise SimMemoryError("SimDbgMemory page sizes differ. This is asking for disaster.")
    ...
    def _apply_object_to_page(self, page_base, mo, page=None, overwrite=True):
        ...
        try:
            ...
            if self.allow_segv:
                ...
                raise
        if self.allow_segv and not page.concrete_permissions & DbgPage.PROT_WRITE:
